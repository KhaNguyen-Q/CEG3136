...
case TITLE: {
    ...
    if (currSelect && !prevSelect) {
        ...
        switch (speedIndex) {
            ...
            case 1: DisplayPrint(1, "Speed: MED");  break;
    ...
    if (currStart && !prevStart) {
        ...
        P1serve = (seed % 2);
        state = SERVE;
        DisplayColor(P1serve ? CYAN : YELLOW);
        DisplayPrint(0, P1serve ? "1P SERVES" : "2P SERVES");
        DisplayPrint(1, "Press Paddle");
...
case SERVE: {
    ...
    if (selectHeld) {
        DisplayColor(RED);
        DisplayPrint(0, "Score");
        char scoreLine[16];
        sprintf(scoreLine, "%02d - %02d", P1score, P2score);
        DisplayPrint(1, scoreLine);
